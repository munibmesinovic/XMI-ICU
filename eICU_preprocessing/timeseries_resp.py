...
def gen_timeseries_file(eICU_path, test=False):
    ...
    # timeseries_resp.to_csv(eICU_path + 'timeseries_resp.csv')
    ...
    #                                               test=test)
    # timeseries_aperiodic.to_csv(eICU_path + 'timeseries_aperiodic.csv')
    #
    ...
    timeseries_resp = pd.read_csv(eICU_path + 'timeseries_resp.csv', index_col = ['patient', 'time'])
    timeseries_aperiodic = pd.read_csv(eICU_path + 'timeseries_aperiodic.csv', index_col = ['patient', 'time'])
    timeseries_lab = pd.read_csv(eICU_path + 'timeseries_lab.csv', index_col = ['patient', 'time'])
    patients = timeseries_resp.index.unique(level=0)
    ...
    for patient_chunk in gen_chunks:
        ...
        if (patient_chunk in list(timeseries_lab.index.unique(level=0))) and (patient_chunk in list(timeseries_lab.index.unique(level=0))) and (patient_chunk in list(timeseries_lab.index.unique(level=0))):
            ...
            merged.clip(lower=-4, upper=4,
                        inplace=True)  # room for +- 3 on each side, as variables are scaled roughly between 0 and 1
