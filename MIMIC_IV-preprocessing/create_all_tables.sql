...
-- change the paths to those in your local computer using find and replace for '/Users/mmesinovic/Desktop/AMI/Data/MIMICIV_data/'.
...
\copy (select subject_id as uniquepid, hadm_id as patienthealthsystemstayid, stay_id as patientunitstayid, hospital_expire_flag as actualhospitalmortality, los as actualiculos from ld_labels as l where l.stay_id in (select * from ld_timeseries_patients) order by l.stay_id) to '/Users/mmesinovic/Desktop/AMI/Data/MIMICIV_data/labels.csv' with csv header
\copy (select * from ld_flat as f where f.patientunitstayid in (select * from ld_timeseries_patients) order by f.patientunitstayid) to '/Users/mmesinovic/Desktop/AMI/Data/MIMICIV_data/flat_features.csv' with csv header
\copy (select * from ld_timeserieslab as tl order by tl.patientunitstayid, tl.labresultoffset) to '/Users/mmesinovic/Desktop/AMI/Data/MIMICIV_data/timeserieslab.csv' with csv header
\copy (select * from ld_timeseries as t order by t.patientunitstayid, t.chartoffset) to '/Users/mmesinovic/Desktop/AMI/Data/MIMICIV_data/timeseries.csv' with csv header